...
void mesh_skinned::apply_skinning(skeleton_geometry const& skeleton)
{
    ...
    for(int k_vertex=0 ; k_vertex<N_vertex ; ++k_vertex)
    {
        ...
        //  vertex(k_vertex) = ...
        vertex(k_vertex)=vec3(0,0,0);
        for(int k_weight=0 ; k_weight<WEIGHTS_PER_VERTEX; ++k_weight){

            skinning_weight w = vertex_weight(k_vertex)[k_weight];
            vertex(k_vertex) += w.weight * (skeleton[w.joint_id].orientation * vertices_original_data[k_vertex] + skeleton[w.joint_id].position);
        }
    }
